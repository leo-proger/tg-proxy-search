...
- `tg_proxy_search/public_source.py` — загрузка локального публичного TXT, скачивание свежей версии и преобразование URL в `Proxy`.
...
Публичной версией `proxies.txt` управляет только pipeline. Обычный интерактивный поиск читает только локальный файл: не скачивает remote-версию, не изменяет файл и не смешивает его с локальным `proxies.json`. Свежий список скачивается и атомарно заменяет локальный файл только при явном выборе «Обновить локальный proxies.txt»; при ошибке последний успешный локальный список должен остаться без изменений.
...
`diagnose.py` — запускать на прокси, которые видны рабочими в Telegram, **с выключенным VPN**. Показывает: TCP-коннект, тип секрета, сырые байты ответа, проверку дайджеста. Выполняет проверку напрямую.
