...
- `tg_proxy_search/public_source.py` — скачивание свежего публичного TXT и преобразование URL в `Proxy` для проверки.
...
Содержимым `proxies.txt` управляет только pipeline. Интерактивная программа может скачать свежую raw-версию и проверить её в памяти, но не должна изменять файл или смешивать его с локальным `proxies.json`. При ошибке или пустом результате парсинга последний успешный публичный список должен остаться без изменений.
