...
2. **Без VPN** — каждый прокси проверяется реальным подключением с вашего IP
...
| `TCP_TIMEOUT` | `15` | Таймаут проверки одного прокси в секундах |
| `PROXY_CHECK_CONCURRENCY` | `8` | Сколько прокси проверять параллельно |
| `MAX_SCAN_MESSAGES` | `1000` | Лимит постов за один запуск парсинга |
| `PROXY_WORKING_RECHECK_HOURS` | `48` | Через сколько часов перепроверять рабочий прокси |
| `PROXY_FAILED_RECHECK_HOURS` | `24` | Через сколько часов перепроверять нерабочий прокси |
| `AUTO_ADD_TO_TELEGRAM` | `false` | Открывать ссылки в Telegram Desktop автоматически |
